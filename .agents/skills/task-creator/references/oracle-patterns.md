...
The oracle (`oracle/solve.sh`) is human-written, demonstrates the answer is producible, and runs without skills (so anything from a skill must be copied into `oracle/` first). Five patterns recur across FrontierPhysics task types.
...
The dominant FrontierPhysics oracle pattern when expected.xlsx contains formulas: write formula strings with openpyxl, then run LibreOffice via the bundled `recalc.py` to populate cached values:
