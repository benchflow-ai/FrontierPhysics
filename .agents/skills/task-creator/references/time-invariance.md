...
A FrontierPhysics task scored against a fixed ground truth must keep producing that ground truth when an agent runs it tomorrow, next month, and a year from now. Tasks that fetch live data are the most common source of time-induced flakes — the world moves on, the answer key doesn't.
