...
## Implication for FrontierPhysics control runs
...
keyword-trigger prompt injection, no audit signal) to tied-best for
FrontierPhysics's
...
4. **Codex implicit-trigger counting** — `detect_implicit_skill_invocation_for_command`
   never appears in the trajectory. To count Codex skill use accurately,
   FrontierPhysics
